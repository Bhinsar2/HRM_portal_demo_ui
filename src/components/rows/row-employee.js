...
const RowEmployee = ({index,data}) =>
{
    ...
    return(
        <tr>
            <td>{index}</td>
            {/* <td><figure className="avatar"> <img src={data.image || `https://ui-avatars.com/api/?name=${encodeURIComponent(data.name || 'User')}&size=64`} alt={data.name}/> </figure></td> */}
            <td>{data.name}</td>
            <td>{data.email}</td>
            <td>{data.mobile}</td>
            ...
            
            {
                ...
                { data.team ?
                    ...
                    {/* <img src={data.team.image || '/assets/icons/team.png'} className='avatar avatar-sm mr-2' alt="Person" width="96" height="96"/> */}
                    {data.team.name}
                </NavLink>
                :
