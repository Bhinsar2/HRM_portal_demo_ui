...
const RowTeam = ({ index, data }) => {
    return (
        <tr>
            <td>{index}</td>
            {/* <td><figure className="avatar"> <img src={data.image || '/assets/icons/team.png'} alt={data.name} /> </figure></td> */}
            <td>{data.name}</td>
            <td>
                {data.leader ?
                    <NavLink to='/' className='badge  badge-primary' style={{ padding: '0px 10px 0px 0px' }}>
                        {/* <img src={data.leader.image || `https://ui-avatars.com/api/?name=${encodeURIComponent(data.leader.name || 'User')}&size=64`} className='avatar avatar-sm mr-2' alt="Person" width="96" height="96" /> */}
                        {data.leader.name}
                    </NavLink>
                    :
